...
-- Add "paid" to revenue_type enum if not exists
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_enum e ON t.oid = e.enumtypid WHERE t.typname = 'revenue_type' AND e.enumlabel = 'paid') THEN
        ALTER TYPE revenue_type ADD VALUE 'paid';
    END IF;
END $$;
...
-- Enable RLS on sub_categories if not already enabled
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'sub_categories' AND rowsecurity = true) THEN
        ALTER TABLE public.sub_categories ENABLE ROW LEVEL SECURITY;
    END IF;
END $$;

-- Drop and recreate policies to avoid conflicts
DROP POLICY IF EXISTS "Sub-categories are viewable by everyone" ON public.sub_categories;
...
-- Add embedded tool fields to products table if not exists
...
-- Create trigger for sub_categories updated_at if not exists
...
-- Insert default sub-categories if they don't exist
INSERT INTO public.sub_categories (name, slug, description, sort_order) 
SELECT * FROM (VALUES
...
) AS new_data(name, slug, description, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM public.sub_categories WHERE slug = new_data.slug);